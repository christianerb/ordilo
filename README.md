...
Ordilo is installable as a PWA (web app manifest + icons, generated by
`scripts/generate-icons.mjs`). On the phone, "Add to Home Screen" gives a
standalone app experience starting on /home. The search bar supports voice
input through the Web Speech API where the browser provides it. Installed
PWAs and unsupported browsers automatically use a secure Realtime
transcription session instead. Speak the question, the final transcript
...
Inviting: the owner creates a share link on the Familie page (valid 14
days, multi-use). Signed-out invitees land on `/invite/[token]`, enter
their email, and the magic-link callback joins them to the family
automatically (`accept_family_invite` RPC, migration 0029) — no
onboarding. One family per account: users who already belong to another
family are rejected with a friendly message.
