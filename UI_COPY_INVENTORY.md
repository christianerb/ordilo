...
| 3.43 | `"Rolle & Beziehung"` / `"Mehreres möglich — z. B. Mutter von Emma und Partnerin von Chris."` / `"von (optional)"` / `"Andere"` / `"Weitere Beziehung"` / `"Fertig"` / `"Entfernen"` / `"Beziehung wählen"` / `"Rolle wählen"` | `src/components/ordilo/member-form.tsx` + `src/components/ordilo/relationship-editor.tsx` + `src/components/ordilo/role-chips.tsx` | Relationship editor: section label, hint, person chips, custom-role chip, add/confirm/remove actions (form) / role chip aria-label (onboarding) |
