...
type Item = { id: string; title: string | null; original_filename?: string | null; deleted_at: string | null };
export function RestoreTrashItems({ documents, tasks }: { documents: Item[]; tasks: Item[] }) {
  ...
  const restoreTask = async (id: string) => { const { error } = await createClient().from("tasks").update({ status: "open", deleted_at: null }).eq("id", id); if (error) return toast.error("Wiederherstellen hat nicht geklappt."); setTaskItems((v) => v.filter((x) => x.id !== id)); toast.success("Aufgabe wiederhergestellt"); };
