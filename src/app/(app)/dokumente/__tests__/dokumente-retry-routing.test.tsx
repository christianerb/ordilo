...
describe("DokumentePage failed-document retry routing", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("routes an OCR-stage failed document's Review Card retry to the OCR endpoint", async () => {
    ...
    // Open the document detail sheet by clicking the document card.
    const docCard = screen.getByTestId("document-card");
    fireEvent.click(docCard);

    // The Review Card error retry button should appear inside the sheet.
