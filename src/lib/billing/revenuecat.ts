...
export async function familyHasPlus(
  familyId: string,
  client: AdminClient = createAdminClient(),
): Promise<boolean> {
  if (process.env.BILLING_ENTITLEMENTS_ENABLED !== "1") return true;
  const { data, error } = await client.rpc("get_family_entitlement_admin", {
    p_family_id: familyId,
  });
  if (error) throw new Error(`Could not resolve family entitlement: ${error.code}`);
  return data?.plan === "plus" || data?.plan === "founding";
}
