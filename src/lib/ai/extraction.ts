...
6. Extrahiere alle relevanten Daten (Fristen, Termine, Abflug, Abfahrt, Ankunft, Zahlungsdaten, Geburtstage) mit Typ und Label. Das Label muss die BEDEUTUNG des Datums beschreiben (z.B. "Zahlungsfrist", "Elternabend", "Gezahlt am") — NIEMALS generische Labels wie "Datum" oder "Termin". Jedes Datum nur EINMAL extrahieren, auch wenn es mehrfach im Dokument steht. WICHTIG: Das Feld date enthält IMMER ein Datum im ISO-Format YYYY-MM-DD (z.B. "2026-07-17", nicht "17.07." oder "Montag") und NIEMALS nur eine Uhrzeit.
   - Eine Uhrzeit gehört zu ihrem Datum: Schreibe sie ins Label dieses Datums, z.B. date "2026-10-12", label "Abfahrt Klassenfahrt 08:15 Uhr" oder label "Elternabend 19:30 Uhr". Lege für eine Uhrzeit NIE einen eigenen Eintrag ohne Datum an. Gehören mehrere Uhrzeiten zum selben Tag (Abflug 19:25, Ankunft 20:55), dürfen es zwei Einträge mit demselben Datum und verschiedenen Labels sein.
   - Das Label ist KURZ: höchstens 5 Wörter (plus Uhrzeit), wie eine Kalenderzeile. Kein Satz, kein "und", kein Ort, keine Adresse — "Abfahrt Klassenfahrt 08:15 Uhr", nicht "Beginn der Klassenfahrt und Abfahrt 08:15 Uhr an der Schule". Meinen Beginn und Abfahrt denselben Zeitpunkt, ist das EIN Eintrag; verschiedene Uhrzeiten (Treffen 08:00, Abfahrt 08:30) bleiben getrennte Einträge.
   - Das Datum des Schreibens selbst (Briefkopf, z.B. "Berlin, 1. September 2026", Rechnungsdatum) mit Typ "document_date" und Label "Briefdatum" (bei Rechnungen "Rechnungsdatum") angeben. Das ist KEIN Termin.
   - Wenn ein Datum nicht eindeutig ist, lass den Eintrag weg.
...
8. Identifiziere Aufgaben (To-dos) mit Frist. Fristen im ISO-Format YYYY-MM-DD angeben. Wenn die Frist kein konkretes Datum ist (z.B. "Montag", "nächste Woche"), setze due_date auf null.
   - Eine Aufgabe ist etwas, das die Familie TUN muss: anmelden, zahlen, überweisen, unterschreiben, mitbringen, abgeben, zurückmelden, kündigen, bestellen, beantragen.
   - Ein Termin, zu dem man nur hingeht (Elternabend, Arzttermin, Abfahrt, Schulfest), ist KEINE Aufgabe. Er steht schon unter den Daten (Schritt 6) und wird nicht zusätzlich als Aufgabe angelegt. Nur wenn es dafür etwas zu tun gibt, wird das eine Aufgabe, z.B. "Zum Elternabend anmelden" mit der Anmeldefrist.
