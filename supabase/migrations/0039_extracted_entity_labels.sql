-- Persist the meaning of an extracted entity alongside its value.
--
-- `extracted_entities` stored only the value, so an amount's or date's label
-- ("Bereits gezahlt", "Zahlungsfrist") was dropped on confirm and the read
-- path in lib/analysis.ts substituted the generic "Betrag" / "Datum". That
-- made every confirmed amount indistinguishable, which in turn let the
-- display-level dedupe collapse two genuinely different amounts (a total and
-- an already-paid part) into one row.
--
-- The column is nullable: rows written before this migration keep NULL and
-- the UI simply shows no sub-label for them.

alter table public.extracted_entities
  add column if not exists label text;

-- Recreate confirm_document so the label is written inside the same
-- transaction as the rest of the confirm. Signature is unchanged from 0035;
-- only the extracted_entities insert gains the label column.
