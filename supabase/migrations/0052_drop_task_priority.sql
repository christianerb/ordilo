...
--
-- Priority (low | medium | high) is no longer used anywhere in the
-- product. The UI no longer collects or displays it, the AI no longer
-- extracts it, and the digest no longer reads it. Keeping a NOT NULL
-- column that nothing writes on purpose would only invite confusion.
--
-- This migration:
--   1. Recreates `confirm_document` without writing `priority` into tasks
--      (signature unchanged — `p_tasks` JSONB may still carry a `priority`
--      key from older clients; it is simply ignored).
--   2. Drops the `priority` column from `public.tasks`.
--
-- Idempotent: `drop column if exists`; the function is replaced with
-- `create or replace`.
