-- Fix: knowledge_nodes.properties_json (person_id / organization_type) was
-- never refreshed on conflict.
--
-- confirm_document upserts a person/organization knowledge_nodes row per
-- label with `on conflict (family_id, type, label) do update`, but the
-- `do update` clause only ever touched `label` and `label_embedding` —
-- `properties_json` stayed whatever the FIRST confirm for that label wrote.
--
-- Concretely: a document mentioning "Max" with no matching family member
-- confirms and creates a person node with properties_json = '{}'. Later,
-- the user assigns "Max" to a real family member on a different document.
-- That second confirm passes the correct person_id, but the upsert leaves
-- the node's properties_json untouched — the link is silently lost, and
-- anything reading the knowledge graph (chat, search) never sees it.
--
-- Fix: on conflict, adopt the incoming properties_json only when it
-- actually carries new information (a person_id, or a non-null
-- organization_type) — never let an unlinked confirm clobber a
-- previously-established link, and never let a linked confirm regress it.
  ...
  loop
    ...
    do update
      set label = excluded.label,
          ...
          -- Adopt the new link only when this confirm actually carries one;
          -- an unlinked re-confirm must not erase a link learned earlier.
          properties_json = case
            when excluded.properties_json ? 'person_id'
              then excluded.properties_json
            else knowledge_nodes.properties_json
          end
  ...
  loop
    ...
    do update
      set label = excluded.label,
          ...
          -- Same rule as persons: only overwrite when the new confirm
          -- actually names a type, so a later untyped mention can't erase
          -- a previously known one.
          properties_json = case
            when excluded.properties_json->>'organization_type' is not null
              then excluded.properties_json
            else knowledge_nodes.properties_json
          end
