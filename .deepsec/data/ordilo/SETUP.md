# Agent setup for `ordilo`

This is a deepsec scanning workspace. Project `ordilo` was just registered
(target: `..`). Setup is incomplete — `data/ordilo/INFO.md`
...
2. **Fill in `data/ordilo/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id ordilo
pnpm deepsec process --project-id ordilo
```
