...
- Realtime-Sprache ist der erste kontextuelle Plus-Moment.
...
2. Android-App mit Package `com.ordilo.app` verbinden.
3. Store-Produkte importieren (sobald ein App Store Connect API Key
   hinterlegt ist; bis dahin reicht die manuelle Pflege).
5. Die Play-Base-Pläne mit `plus` verbinden, sobald Android angebunden ist.
...
```text
EXPO_PUBLIC_REVENUECAT_IOS_API_KEY=appl_iSBuhdmQudhJwZQNXeznSMNpLlb
...
1. Migration `0081_family_entitlements.sql` in Supabase anwenden.
