...
| `APP_BASE_URL` | empfohlen | Absolute App-URL für Links in der Mail, in Produktion `https://ordilo.de`. Fallback: Request-Origin. Wird auch für den Login-Link bei Familien-Einladungen genutzt (`/invite/[token]`) — dort schützt sie gegen Host-Header-Injection und sollte in Produktion gesetzt sein. |
